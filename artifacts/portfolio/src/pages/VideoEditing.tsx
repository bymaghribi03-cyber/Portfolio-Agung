          ...
          {reelsProjects.map((reel, i) => (
            ...
            >
              ...
              <div className="relative overflow-hidden" style={{ aspectRatio: "9/16" }}>
                ...
                {/* Instagram badge */}
                <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-gradient-to-br from-[#f09433] via-[#e6683c] to-[#bc1888] flex items-center justify-center text-white shadow-md">
                  <FaInstagram size={10} />
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                  <p className="text-white text-[10px] font-semibold leading-snug line-clamp-2">{reel.title}</p>
                </div>
              </div>

              {/* Card footer */}
              <div className="p-2">
                <p className="text-dark-green font-semibold text-[11px] leading-snug line-clamp-1 mb-1.5">{reel.title}</p>
                <a
                  href={reel.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-[#f09433] via-[#e6683c] to-[#bc1888] text-white text-[10px] font-medium rounded-full hover:opacity-90 transition-opacity duration-200"
                  data-testid={`reel-watch-btn-${reel.id}`}
                >
                  <FaInstagram size={9} /> View
                </a>
              </div>
            </motion.div>
          ))}
